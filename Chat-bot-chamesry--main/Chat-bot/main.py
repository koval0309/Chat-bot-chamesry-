...
def main(message):
    bot.send_message(message.chat.id, "Привіт, я твій помічник з хімії! ")
...
def tabl(message):
    if message.text.lower() == "h":
        ...
        bot.send_message(message.chat.id, "Калій (K) — хімічний елемент, належить до групи лужних металів, символ К, атомний номер 19; атомна маса 39,098.М'який сріблясто-білий метал. Хімічно дуже активний, сильний відновник, на повітрі легко окиснюється. Відкритий англійським хіміком Гемфрі Деві 1807 року. Густина 0,856. Температура плавлення 63,55C, температура кипіння 760C. Твердість за Брінеллем 400 кПа. Металічний калій легко ріжеться ножем.")
    elif message.text.lower() == "ca":
        bot.send_message(message.chat.id, "Кальцій, також вапнець ва́пень, ва́пник або вап (Ca)— хімічний елемент з атомним номером 20 та його проста речовина, що належить до лужноземельних металів, сріблясто-білий м'який метал, хімічно активний, легко окиснюється. Густина 1,55, температура плавлення 842°C, температура кипіння 1491°C. Твердість кальцію за Брінеллем 200–300 МПа.")
    elif message.text.lower() == "sc":
        bot.send_message(message.chat.id, "Скандій — хімічний елемент з символом Sc та порядковим номером в Періодичній системі 21.Сріблясто-білий метал, належить до групи перехідних металів, він іноді класифікується як рідкісноземельний елемент разом з ітрієм та лантаноїдами. Густина 3,020; tплав 1541°С; tкип 2850°С. Існує у двох кристалічних модифікаціях: α-Sc з гексагональною ґраткою типу магнію, β-Sc з кубічною об’ємноцентрованою ґраткою, температура переходу α↔β 1336°C.")
